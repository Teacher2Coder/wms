...
const WarehouseCards = ({
  ...
  const { canManage, isAdmin } = useAuth(); // Only Admin and Manager can create warehouses
  ...
  return (
    <div>
      ...
      {warehouses.length === 0 ? (
        /* Empty State */
        <motion.div variants={itemVarients} className="text-center py-20">
          <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-12 max-w-md mx-auto shadow-glow">
            ...
            <p className="text-accent-600 mb-6">
              {canManage
                ? "Start by adding your first warehouse to get started."
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-primary-50/80">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-accent-800">
                    Warehouse
                  </th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-accent-800">
                    Location
                  </th>
                  <th className="px-6 py-4 text-center text-sm font-semibold text-accent-800">
                    Total
                  </th>
                  <th className="px-6 py-4 text-center text-sm font-semibold text-accent-800">
                    Available
